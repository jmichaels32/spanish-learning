...
englishWord.dispatchEvent(new dom.window.KeyboardEvent("keydown", { key: "Enter", bubbles: true, cancelable: true }));
assert.equal(document.activeElement, spanishWord);
assert.equal(document.querySelector("#google-translate-link"), null);
assert.equal(document.querySelectorAll("[data-add-word-accent]").length, 7);
document.querySelector("[data-add-word-accent='ñ']").click();
assert.equal(spanishWord.value, "ñ");
spanishWord.value = "la meta";
spanishWord.dispatchEvent(new dom.window.Event("input", { bubbles: true }));
assert.equal(document.querySelector("#add-word-form button[type='submit']").disabled, false);
...
assert.match(document.querySelector("#vocab-form-word-count").textContent, /1 word saved/);
assert.equal(document.querySelector("#add-word-form button[type='submit']").disabled, true);
